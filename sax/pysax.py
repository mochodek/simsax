...
class SAXModel(object):
    def __init__(self, window=None, stride=1,
                 nbins=None, alphabet=None, epsilon=0.05):
        """
        ...
        # vecs = list(map(lambda a, b: (a+b)/2, zip(cps, cps[1:]))) ## taking mean may not be accurate
        vecs = list(starmap(lambda a, b: (a + b) / 2, zip(cps, cps[1:])))  ## taking mean may not be accurate
        vecs[0] = cps[1]
        vecs[-1] = cps[-2]
    ...
    def whiten(self, window_signal):
        ...
        if np.std(s) < self.epsilon:
            return [0 for entry in s]
        mu, sd = np.mean(s), np.std(a=s, ddof=1)
        return (s - mu) / sd
    ...
    def paa_window(self, window_signal):
        ...
        xs = list(map(lambda ss: np.sum(ss) / float(binsize), self.binpack(s)))
        return xs
